...
class learning_loop():
    def __init__(self, N_EPOCH,model,device,train_data,val_data,test_data,optimizer,criterion,writer, mode ) -> None:
    ...
    def train(self):
        for epoch in range(1, self.N_EPOCH+1):
            for (_,train) , (_,test) in zip(enumerate(self.train_data, 0),enumerate(self.val_data, 0)):
                ...
                # X_train_ts, y_train_ts = X_train_ts.to(self.device), y_train_ts.type(th.LongTensor).to(self.device)
                # X_val_ts, y_val_ts = X_val_ts.to(self.device), y_val_ts.type(th.LongTensor).to(self.device)
                ...
                # y_train_ts = y_train_ts.view(-1,1)
                # y_val_ts = y_val_ts.view(-1,1)
                
                out = self.model(X_train_ts.to(th.float32))      
                # loss = criterion(out, y_train_ts.to(th.float32))
            ...
            self.writer.add_scalar('Loss/test', loss_val.item(), epoch)  
            self.writer.add_scalar('Loss/train', loss.item(), epoch)
            
            if self.mode == 'classification':
                self.writer.add_scalar('Accuracy/test', acc_val*100, epoch)
                self.writer.add_scalar('Accuracy/train', acc*100 , epoch)
    ...
    def test(self):
        ...
        for (i,test) in enumerate(self.test_data, 0):
            ...
            # X_test_ts, y_test_ts = X_test_ts.to(self.device), y_test_ts.type(th.LongTensor).to(self.device)
